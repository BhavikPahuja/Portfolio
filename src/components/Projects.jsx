...
const projects = [
  {
    ...
    image: (<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-20 hover:scale-110 transition mb-3">
...
export default function Projects() {
  return (
    <section id="projects" className="min-h-screen text-white px-6 py-24">
      <motion.h2
        initial={{ opacity: 0, y: 40 }}
        whileInView={{ opacity: 1, y: 0 }}
        transition mb-3={{ duration: 0.7 }}
        className="text-5xl font-bold mb-12 text-center mb-40"
      >
        Projects
      </motion.h2>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
        {projects.map((proj, i) => (
          <motion.a
            ...
            whileInView={{ opacity: 1, y: 0 }}
            transition mb-3={{ duration: 0.5 + i * 0.2 }}
            className="bg-white/10 rounded-xl shadow-2xl p-10 flex flex-col items-center hover:bg-white/20 transition mb-3-all duration-300 group"
          >
            {proj.image}
            <h3 className="text-2xl font-semibold mb-2 text-purple-200">{proj.title}</h3>
            <p className="text-white/80 text-center">{proj.desc}</p>
          </motion.a>
        ))}
      </div>
    </section>
